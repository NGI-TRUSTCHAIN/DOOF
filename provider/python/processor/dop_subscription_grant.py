...
class DopSubscriptionGrantProcessor(ProcessorProvider):

    def __init__(self):
        super().__init__()
        self._config = ""
        self._event_type = "dop_subscription_grant"
    ...
    def handle_event(self, event: DopEvent, envs: ProcessorEnvs) \
        ...
        if self._event_type == event.header.event:
            return self._handle_dop_subscription_grant(event, envs)
        
        return DopError()


    def _handle_dop_subscription_grant(self, event: DopEvent, envs: ProcessorEnvs) -> DopError:
        ...
        perr, tx_hash = blk.subscriptionGrant(subscription.blk_address, publisher.blk_password)
