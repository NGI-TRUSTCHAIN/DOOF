...
"event": "event_set",
"params":
     {"transaction_id": "0xae715513be3f50a38830174c75bc0b1061f3939cb2ca07c30524cc49a4e52de4", 
    "event_set": [{"event_id": "LogSubscriptionDelete", "data":
...
class LogSubscriptionDeleteProcessor(ProcessorProvider): 
    ...
    def __init__(self):
        ...
        self._event_id_content = "LogSubscriptionDelete"
        self._trigger_event = "dop_product_unsubscribe"
    ...
    def handle_event(self, event: DopEvent, envs: ProcessorEnvs) \
        ...
        # HERE STARTS THE LOGIC TO COMPLETE SUBSCRIPTION GRANT
        
        sid_blk_addr = data.get('subscription_address')     # cross-check with event_trans
        subscription_id = data.get('subscription_id')
        product_id = data.get('product_id')
        subscriber_id = user.id             # NOTE can cross check on db query on subscriber_blk_addr  
        ...
        envs.data.push(ProductSubscription.__name__, db_subscription)
        ...
        # Delete all references to this subscription from db 


        perr = envs.db_provider.delete_product_subscription(
                subscription_id, 
                subscriber_id,
                product_id
            )
        ...
        # after deleting the subscription in the database, delete transaction
        perr = db.delete_transaction(transaction_id) 
        if perr.isError():
            # if there is an error when deleting transaction, 
            # everything is rolled back 
